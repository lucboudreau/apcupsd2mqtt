...
MQTT_HOSTNAME = os.getenv('MQTT_HOSTNAME', 'apcupsd-mqtt')
MQTT_INTERVAL = float(os.getenv('MQTT_INTERVAL', 15))
MQTT_TOPIC_PREFIX = os.getenv('MQTT_TOPIC_PREFIX', '/apcupsd')
...
t.add_row(['MQTT_INTERVAL', MQTT_INTERVAL])
t.add_row(['UPS_ALIAS', UPS_ALIAS])
t.add_row(['APCUPSD_HOST', APCUPSD_HOST])
...
def main():
    ...
    HOSTNAME = ups.get('HOSTNAME', MQTT_HOSTNAME)

    first_run = True
    logger.info("Printing first submission for debug purposes:")


    result = pyfiglet.figlet_format(HOSTNAME)
    print( result )
    while True:
...
#        watts = float(os.getenv('WATTS', ups.get('NOMPOWER', 0.0))) * 0.01 * float(ups.get('LOADPCT', 0.0))
        ...
        for k in ups_data:
            topic_id = MQTT_TOPIC_PREFIX + "/" + UPS_ALIAS + "/" + str(k)
            if first_run:
